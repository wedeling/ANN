...
if eddy_forcing_type == 'ann_tau_ortho':
    ...
    if on_the_fly == True:
        batch_size = 512
        dE_ann.set_batch_size(batch_size)
        dZ_ann.set_batch_size(batch_size)
        batch_counter = 0
...
if restart == True:
    ...
    z_n_HF, e_n_HF, u_n_HF, s_n_HF, v_n_HF, o_n_HF, sprime_n_HF, zprime_n_HF = compute_qoi(P_LF*w_hat_n_HF, verbose=False)
    ...
    tau_E_n = np.tanh(dE/e_n_LF)*np.sign(src_E)
    tau_Z_n = np.tanh(dZ/z_n_LF)*np.sign(src_Z)
...
for n in range(n_steps):    
    ...
    #ANN surrogate of the orthogonal pattern forcing
        ...
        z_n_HF, e_n_HF = compute_ZE(P_LF*w_hat_n_HF, verbose=False)
        dE_ref = e_n_HF - e_n_LF
        dZ_ref = z_n_HF - z_n_LF

        #features
        X_feat = np.array([z_n_LF, e_n_LF, u_n_LF, s_n_LF, v_n_LF, o_n_LF, tau_E_n*sprime_n_LF, tau_Z_n*zprime_n_LF])
        ...
        if on_the_fly == True:
            X_on_the_fly[batch_counter, :] = X_feat
            dE_on_the_fly[batch_counter] = e_n_HF - e_n_LF
            dZ_on_the_fly[batch_counter] = z_n_HF - z_n_LF
            batch_counter += 1
            
            if batch_counter == batch_size:
                batch_counter = 0
                #print('on-the-fly update')
                dE_ann.batch(X_on_the_fly, dE_on_the_fly)
                dZ_ann.batch(X_on_the_fly, dZ_on_the_fly)
